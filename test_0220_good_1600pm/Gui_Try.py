...
class Mywin_xx(wx.Frame):
   ...
   def __init__(self, parent, title):
      super(Mywin_xx, self).__init__(parent, title=title, size=(530,450))
      self.InitUI()
   def InitUI(self):
      self.count = 0
      pnl = wx.Panel(self)
      ...
      self.gauge = wx.Gauge(pnl, range=80,pos=(20,350),size=(406, 20), style = wx.GA_SMOOTH)
      self.text = wx.TextCtrl(pnl, size=(744,100),pos=(5,30), style=wx.EXPAND|wx.TE_MULTILINE)
      self.btn1 = wx.Button(pnl, label="Deploy Static Content(select ZIP/tar.gz)")
      ...
      hbox2.Add(self.btn1, proportion=1, flag=wx.RIGHT, border=10) # this one is file open window
      vbox.Add(hbox2, proportion=1, flag=wx.ALIGN_CENTRE)  # this is file open window
      ...
      #hbox2.Add(self.text, proportion=5, flag=wx.ALIGN_CENTRE, border=10)
      hbox1.Add(self.gauge, proportion=10, flag=wx.ALIGN_CENTRE, border=20) # this is good one
      pnl.SetSizer(vbox)
