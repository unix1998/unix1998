...
destination="d:\opt\web_Roots"
destination2="D:\opt\web_Roots"
destination2_list=['rams']
env_string="ssss"
...
def get_app_string(full_path_name):
    ...
    try:
       return  full_path_name.split('\\')[4]
    except IndexError as e:
       print (" wrong files was selected ")
       print (e)
       #print (sys.exc_type)
       #print (" wrong files was selected ")
	...
	# changed to 4 on March 13 
################################################################

################################################################
def get_env_string(full_path_name):
...
def set_running_flag():
 ...
 if os.path.isfile(".\\static_deploy.conf"):
   ...
   dlg_11 = wx.MessageDialog(frame_1, 'no config file found , please add it, exit now ','', wx.OK | wx.CANCEL | wx.ICON_ERROR)
...
def init_conf(env_string1):
 ...
 destination = config.get(env_string1,'destination')
 destination2 = config.get(env_string1,'destination2')
 destination2_list = config.get(env_string1,'Apps_list2')
 backup_folder = config.get(env_string1,'backup_folder')
 log_folder = config.get(env_string1,'log_folder')
 temp_folder = config.get(env_string1,'temp_folder')
 testing_2 = config.get(env_string1,'Apps_list')
 testing_3= config.get(env_string1,'Apps_list2')
 testing_123=testing_2+","+testing_3
 #ff1=open(running_flag,"w+")
 #ff1.write("we are running")
 #ff1.close
 for aa in testing_123.split(','):
    #print (aa)
...
def backup_app(app_name):
    ...
    #print (today2)
    #print (destination2_list)
    #print (app_name)
    destination2_list_2=destination2_list.split(",")
    if app_name in destination2_list_2:
       destination=destination2
	# handle extra destiname , rams , ramsW and so on 
    #print (destination) 
...
def unzip_1(zip_f):
  ...
  destination2_list_2=destination2_list.split(",")
  if app_name in destination2_list_2:
    #path_name_11=destination2
    destination=destination2
  # handle 2nd destination
...
##############################need by WX##################################################
###################################add a diaglog box######################################################
# blank area for new add function ##
...
class RedirectText(object):
    def __init__(self,aWxTextCtrl,log_file_name):
...
class TestThread(Thread):
    ...
    def run(self):
        ...
        for i in range(15):
            ...
            wx.CallAfter(pub.sendMessage, "update", list1=('1','a'), list2=('2','b'))
			#list1=('1','a'), list2=('2','b')
            #wx.CallAfter(pub.sendMessage, "update", msg="")
...
class Mywin_xx(wx.Frame):
   ...
   def InitUI(self):
      ...
      self.text = wx.TextCtrl(pnl, size=(740,268),pos=(4,29),  style=wx.TE_MULTILINE|wx.TE_READONLY|wx.HSCROLL)
      ...
      menubar = wx.MenuBar()
      fileMenu = wx.Menu()
      fitem = fileMenu.Append(wx.ID_EXIT, 'UAT', 'Quit application')
      fitem = fileMenu.Append(wx.ID_EXIT, 'UAT2', 'Quit application')
      fitem = fileMenu.Append(wx.ID_EXIT, 'PROD', 'Quit application')
   ...
   def OnClick(self, event1):
     ...
     def add_diag_box(string_1):
           global going_on
           #app_2 = wx_1.App(False)
           #app_2 = wx.App(False)
           #frame_1 = wx.Frame(None, -1, 'win.py',size=(550,380))
           if (env_string.lower() in ["uat","uat2","prod" ]) : 
               
	
                the_string="Do you want deploy   " + string_1 + " ??" 
                dlg_111 = wx.MessageDialog(self, the_string,'', wx.YES_NO | wx.ICON_QUESTION)
                #val = dlg_111.ShowModal()
                result = dlg_111.ShowModal()
                #dlg_11.Show()
                if result == wx.ID_YES:
                     going_on=True
                else:
                ...
                the_string="wrong file was selected , please reselect "
                dlg_111 = wx.MessageDialog(self, the_string,'', wx.YES_NO | wx.ICON_QUESTION)
                result = dlg_111.ShowModal()
                going_on=False
           dlg_111.Destroy
     ...
     if dlg.ShowModal() == wx.ID_OK:
        ...
        if going_on :
          ...
          #sl = StreamToLogger(app_name_01,stdout_logger, logging.DEBUG) 
          sl = StreamToLogger(app_name_01,stdout_logger, app_log_level)
          sys.stdout = sl
          #sys.stdout=self.text() # test redirect to diag
          stderr_logger = logging.getLogger('STDERR')
          sl = StreamToLogger(app_name_01,stderr_logger, logging.DEBUG) 
          sys.stderr = sl
          #logger1.addHandler(fileh_x)
		  # redirect text here
		  
		  ##########################################
          redir=RedirectText(self.text,out_file_name)
          sys.stdout=redir
		  ########################################
          ...
          self.text.SetValue(put_string)
          TestThread()
		  # commented out on MArch 13 , 12:00 noon , as a test 
          ...
          self.text.SetValue(' re select a src archive file')
     dlg.Destroy
	 ###########  test March 13, 16:05 #####
...
set_running_flag()
# set running flag only , initialization moved into dialog event #####
#init_conf()
