...
class StreamToLogger(object):
 ...
 logging.basicConfig(
   ...
   filename=log_folder +"\\"+"\\out"+today1,
   filemode='a'
 )
...
class Mywin_xx(wx.Frame):
   ...
   def __init__(self, parent, title):
      super(Mywin_xx, self).__init__(parent, title=title, size=(630,530))
      self.InitUI()
   def InitUI(self):
      self.count = 0
      font1 = wx.Font(pointSize = 14, family = wx.DEFAULT,
      ...
      self.gauge = wx.Gauge(pnl, range=80,size=(338, 20), style=wx.GA_HORIZONTAL)
      self.text = wx.TextCtrl(pnl, size=(688,100),pos=(5,30), style=wx.TE_MULTILINE)
...
ex1 = wx.App()
Mywin_xx(None,'Static Deployment')
#ex1.MainLoop()
